...
1. **风险提示**：量化交易涉及高风险，市场波动可能导致资金部分或全部损失。本项目仅供技术研究与学习参考，不构成任何投资建议。
2. **实盘谨慎**：用户在进行实盘交易前，应充分理解策略逻辑，并建议先在模拟盘进行充分测试。因代码缺陷、系统故障、网络延迟或API错误导致的任何交易损失，作者及贡献者概不负责。
3. **个人责任**：用户需对自己的交易行为和配置参数负全部责任。使用本项目即表示您已阅读并同意承担所有相关风险。
4. **法律合规**：请确保您的交易行为符合当地法律法规。
