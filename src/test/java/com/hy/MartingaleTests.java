...
public class MartingaleTests {
    ...
    /**
     * 生成马丁格尔每单价格和保证金序列（BigDecimal精度版）
     *
     ...
     * @param leverage         杠杆倍数
     * @param baseMargin       初始加仓单保证金
     * @param direction        多单 LONG / 空单 SHORT
     * @return 每单信息列表
     */
    public static List<OrderLevel> generateOrderPlan(
        ...
        for (int i = 0; i < maxAddCount; i++) {
            // 本次加仓保证金 = baseMargin * (amountMultiplier ^ i) * leverage
            BigDecimal multiplierPower = amountMultiplier.pow(i);
            BigDecimal margin = baseMargin.multiply(multiplierPower).multiply(leverage);
    ...
    public static void main(String[] args) {
        ...
        List<OrderLevel> plan = generateOrderPlan(
                entryPrice,
                baseStep,
                maxAddCount,
                amountMultiplier,
        ...
        BigDecimal reduce = plan.stream().map(level -> level.margin).reduce(BigDecimal.ZERO, BigDecimal::add);
        System.out.println("总投入保证金: " + reduce.setScale(4, RoundingMode.HALF_UP));
    }
}
