...
public class MartingaleStrategyService {
    ...
    public void loadDefaultConfig() {
        ...
        STRATEGY_CONFIG_MAP.put(SymbolEnum.BTCUSDT.getCode(), new MartingaleStrategyConfig(true, SymbolEnum.BTCUSDT.getCode(), Direction.LONG, 4, 1, 100, 0.5, 2.0, BigDecimal.valueOf(100.0), 20, 1.1, 1.1, "0.0001", true, 0.5));
        // ETH配置：杠杆100倍，跌1%加仓，止盈2%
        STRATEGY_CONFIG_MAP.put(SymbolEnum.ETHUSDT.getCode(), new MartingaleStrategyConfig(true, SymbolEnum.ETHUSDT.getCode(), Direction.LONG, 2, 2, 100, 2.0, 3.0, BigDecimal.valueOf(100.0), 15, 1.1, 1.1, "0.01", false, 1.0));
    ...
    private void handleSuccessfulOrder(MartingalePlaceOrderParam orderParam, BitgetPlaceOrderResp orderResult) {
        try {
            ...
            //首单与加仓单间隔
            if (config.getFirstAddPositionPercentThreshold() != null && config.getFirstAddPositionPercentThreshold() > 0) {
                if (Direction.LONG == config.getDirection()) {
                    entryPrice = AmountCalculator.decrease(entryPrice, BigDecimal.valueOf(config.getFirstAddPositionPercentThreshold()), config.getPricePlace());
                } else {
                    entryPrice = AmountCalculator.increase(entryPrice, BigDecimal.valueOf(config.getFirstAddPositionPercentThreshold()), config.getPricePlace());
                }
            }
